var leafletControlGeocoder = (function (L) {
        ...
        _proto.getJSON = function getJSON$1(url, params, cb, context) {
            getJSON(url, params, function (data) {
                ...
                if (data.response.view && data.response.view.length) {
                    for (
                        var i = 0;
                        i <= data.response.view[0].result.length - 1;
                        i++
                    ) {
    ...
    var HEREv2 = /*#__PURE__*/ (function () {
        ...
        _proto2.geocode = function geocode(query, cb, context) {
            ...
            this.getJSON(
                this.options.serviceUrl + "/discover",
                params,
                cb,
                context
            );
                ...
                if (data.results && data.results.length) {
                    for (var i = 0; i < data.results.length; i++) {
                        ...
                        if (loc.annotations && loc.annotations.bounds) {
                            bbox = L__namespace.latLngBounds(
                                L__namespace.latLng(
                                    loc.annotations.bounds.northeast
                                ),
                                L__namespace.latLng(
                                    loc.annotations.bounds.southwest
                                )
                            );
                        } else {
                            bbox = L__namespace.latLngBounds(center, center);
    ...
    var EventedControl = function EventedControl() { // eslint-disable-next-line @typescript-eslint/no-unused-vars
        // empty
    };
    ...
    var GeocoderControl = /*#__PURE__*/ (function (_EventedControl) {
        ...
        _proto._geocode = function _geocode(suggest) {
            ...
            var cb = function cb(results) {
                if (requestCount === _this3._requestCount) {
                    ...
                    _this3.fire(
                        suggest ? "finishsuggest" : "finishgeocode",
                        _event
                    );
        ...
        _proto._geocodeResultSelected = function _geocodeResultSelected(
            geocode
        ) {
            var event = {
                geocode: geocode,
